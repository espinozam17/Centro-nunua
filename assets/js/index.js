...
let nombresDentales = document.getElementById("dental");

dental.forEach(
  (paciente) =>
    (nombresDentales.innerHTML += `${paciente.hora} - ${paciente.especialista} - ${paciente.paciente} - ${paciente.rut}-${paciente.prevision}  <br>`)
);

let nombresNunoa = document.getElementById("nunoa");
dental.forEach(
  (paciente) => (nombresNunoa.innerHTML += `${paciente.paciente}<br>`)
);
traumatologia.forEach(
  (paciente) => (nombresNunoa.innerHTML += `${paciente.paciente}<br>`)
);
radiologia.forEach(
  (paciente) => (nombresNunoa.innerHTML += `${paciente.paciente}<br>`)
...
let isapreDental = dental.filter(function (dental) {
  return dental.prevision == "ISAPRE";
});
let nombresIsapre = document.getElementById("isapre");
isapreDental.forEach(
  (paciente) =>
    (nombresIsapre.innerHTML += `${paciente.paciente}- ${paciente.prevision} <br>`)
);

let fonasaTraumatologia = traumatologia.filter(function (traumatologia) {
  return traumatologia.prevision == "FONASA";
});
let nombresFonasa = document.getElementById("fonasa");
fonasaTraumatologia.forEach(
  (paciente) =>
    (nombresFonasa.innerHTML += `${paciente.paciente}- ${paciente.prevision} <br>`)
);